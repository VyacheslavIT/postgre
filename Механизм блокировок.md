...
Обе транзакции начинают обновление таблицы одновременно, и блокируют друг друга из-за конфликта блокировок. Допустим, транзакция A блокирует строку X для обновления, а транзакция B блокирует строку Y для обновления. Затем транзакция A пытается получить блокировку на строку Y, в то время как транзакция B пытается получить блокировку на строку X. Это приводит к ситуации, когда ни одна из транзакций не может продолжить выполнение из-за блокировок, и возникает deadlock.

Обе транзакции обновляют одну и ту же строку без использования WHERE, одновременно или поочередно. В этом случае, если одна из транзакций не завершит операцию обновления, другая транзакция может быть заблокирована, ждать завершения операции и также вызвать ошибку deadlock.

В обоих случаях, база данных обнаруживает deadlock;
