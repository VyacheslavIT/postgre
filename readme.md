...
> Уровень изоляции READ COMMITTED не позволяет читать грязные данные и пока первая сессия не закончит выполнять свою транзакцию ,т.е. добавлять новую запись в таблицу, вторая сессия не увидит новую запись в таблице, так первая сессия может откатиться.
